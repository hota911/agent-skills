...
metadata:
  ...
  source-author: k16shikano (@golden_lucky)
  reference: https://x.com/f4ah6o/status/2067215523067400244
  note: 鹿野桂一郎 (k16shikano / @golden_lucky) 氏の gist を元に整形・スキル化した派生物。原著者が gist の改変・再配布を公に歓迎している。
---

# 日本語技術文書の文章規範

> **出所・クレジット**: 本スキルの文章規範は、鹿野桂一郎（[k16shikano](https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d) / [@golden_lucky](https://x.com/golden_lucky)）氏が公開した gist を元に、Agent Skill として整形した派生物である。原著者は gist の改変・再配布を公に歓迎している（[出典スレッド](https://x.com/f4ah6o/status/2067215523067400244)）。
...
- 太字の `**` を全角約物（「」『』（）。、など）の外側に隣接させない。CommonMark は「直前が約物・直後が文字」の `**` を閉じと判定せず（開き側も対称の規則）、`は**「引用」**である` は生の `**` がそのまま表示される。括弧つきの語句を強調するときは、括弧ごと太字にせず中身だけを太字にする（`「**中身**」`）。
